...
def step_impl(context):
    ...
    assert context.output.splitlines()[1] == "git config --global user.email World@picklerick.com"
    assert context.output.splitlines()[2] == "git config --global init.defaultbranch=main"

@then('we configure git default template')
def step_impl(context):
    assert context.output.splitlines()[3] == "git config --global commit.template ~/.config/gitmsg.txt"
